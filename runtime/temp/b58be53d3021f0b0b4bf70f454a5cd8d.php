<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:65:"D:\xampp\htdocs\public/../application/admin\view\Lunbo\index.html";i:1510295148;}*/ ?>
﻿<!DOCTYPE html>
<html lang="en">
<head>
<title>轮播列表</title>
...
<script>
  function lay(id){
    layer.confirm(
      '确认删除？', 
      {btn: ['确定', '取消'],
      btn1:function(index, layero){window.location.href='/public/admin/lunbo/del?id='+id}
      }
      );
    return false;
  }
...
<body>
 <div class="wrap">
  <div class="page-title">
    ...
    <span class="modular fr"><a href="/public/admin/lunbo/add_show" class="pt-link-btn">+添加轮播</a></span>
  </div>
  <table class="list-style Interlaced">
   <tr>
    ...
    <?php if(is_array($p->data) || $p->data instanceof \think\Collection || $p->data instanceof \think\Paginator): $i = 0; $__LIST__ = $p->data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$x): $mod = ($i % 2 );++$i;?>
      ...
      <td class="center">
       <a href="/public/admin/lunbo/updata_show?id=<?php echo $x['id']; ?>" title="编辑"><img src="__STATIC__/admin/images/icon_edit.gif"/></a>
...
$(document).ready(function(){
  $("input").on("click",function(){
       ...
       if($(this).next().attr("vvv")==1)
        ...
        $.post("/public/admin/lunbo/updata_index_show",{'val':$(this).next().attr('vvv'),'id':$(this).next().attr('setid')},function(result){
              console.log(result);
            });
      });
});
